...
pdf('fitness_migr_vs_within.pdf')
for (t in 1:nsim){
  print(t)
  # cnt <- rep(0,64)
  tre.sm <- trees.sm[[t]]
    ...
    if(length(nodes_clade)>25 & max(dates)-min(dates)>0.5){
      for(gc in fam$gd_ch){
        ...
        for(c in children){
          ...
          if(length(f)<4) next()
          if(length(check_within(f, reg, GS))==length(f)) {fitness_within <- c(fitness_within, Fitness[strtoi(f)]-Fitness[c]); count <- count+1} #if all nodes in the same region
        }
      }
    }
  }
